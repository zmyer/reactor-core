...
public class FluxTests extends AbstractReactorTest {
	...
	public void testDoOnEachSignal() {
		...
		assertThat(signals.get(2).isOnComplete()).withFailMessage("onComplete expected").isTrue();
		assertThat(values.get(0)).withFailMessage("1st onNext value unexpected").isEqualTo(1);
		assertThat(values.get(1)).withFailMessage("2nd onNext value unexpected").isEqualTo(2);
	}
	...
	public void testDoOnEachSignalWithError() {
		...
		assertThat(signals.get(0).isOnError()).withFailMessage("onError expected").isTrue();
		assertThat(signals.get(0).getThrowable()).withFailMessage("plain exception expected").hasMessage("foo");
	}

	@Test
	public void testDoOnEachSignalNullConsumer() {
		assertThatNullPointerException()
				.isThrownBy(() -> Flux.just(1).doOnEach(null));
	}
	...
	public void testComposeFromSingleValue() throws InterruptedException {
		...
		await(s, v -> assertThat(v).isEqualTo("Goodbye then!"));
	}
	...
	public void testComposeFromMultipleValues() throws InterruptedException {
		...
		await(5, s, v -> assertThat(v).isEqualTo(15));
	}
	...
	public void testComposeFromMultipleFilteredValues() throws InterruptedException {
		...
		await(2, s, v -> assertThat(v).isEqualTo(4));
	}
	...
	public void testComposedErrorHandlingWithMultipleValues() throws InterruptedException {
		...
		await(5, s, v -> assertThat(v).isEqualTo(10));
		assertThat(exception.get()).as("error triggered").isTrue();
	}
	...
	public void testReduce() throws InterruptedException {
		...
		await(1, s, v -> assertThat(v).isEqualTo(120));
	}
	...
	public void testStreamBatchesResults() {
		...
		assertThat(batchCount.get()).as("batchcount").isEqualTo(1);
		assertThat(count.get()).as("count").isEqualTo(15);
	}
	...
	public void testHandlersErrorsDownstream() throws InterruptedException {
		...
		await(2, s, v -> assertThat(v).isEqualTo(3));
	...
	public void promiseErrorCountCannotExceedOne() {
		...
		assertThat(deferred.getError()).isSameAs(error);
	}
	...
	<T> void await(Flux<T> s, Consumer<T> assertion) throws InterruptedException {
		await(1, s, assertion);
	}

	<T> void await(int count, final Publisher<T> s, Consumer<T> assertion) throws InterruptedException {
	...
	public void analyticsTest() throws Exception {
		...
		assertThat(result.block(Duration.ofSeconds(5))
		                 .doubleValue()).isGreaterThanOrEqualTo(avgTime * 0.6);
	}
	...
	public void konamiCode() throws InterruptedException {
		...
		assertThat(res.get(0)).as("res 0").isFalse();
		assertThat(res.get(1)).as("res 1").isTrue();
		assertThat(res.get(2)).as("res 2").isFalse();
		assertThat(res.get(3)).as("res 3").isFalse();
		assertThat(res.get(4)).as("res 4").isFalse();
		assertThat(res.get(5)).as("res 5").isFalse();
		assertThat(res.get(6)).as("res 6").isFalse();
		assertThat(res.get(7)).as("res 7").isFalse();
		assertThat(res.get(8)).as("res 8").isFalse();
		assertThat(res.get(9)).as("res 9").isFalse();
		assertThat(res.get(10)).as("res 10").isFalse();
		assertThat(res.get(11)).as("res 11").isFalse();
	}
	...
	public void shouldNotFlushStreamOnTimeoutPrematurely() throws Exception {
		...
		assertThat(NUM_MESSAGES / BATCH_SIZE * TOLERANCE >= batchesDistribution.get(BATCH_SIZE) * TOLERANCE)
				.withFailMessage("Less than 90% (" + NUM_MESSAGES / BATCH_SIZE * TOLERANCE +
	...
	public void prematureFlatMapCompletion() throws Exception {
		...
		assertThat(res).isEqualTo(2_000_000);
	}
	@Test
	...
	public void sequenceEqual() throws Exception {
		boolean res = Mono.sequenceEqual(Flux.just(1, 2, 3), Flux.just(1, 2, 3))
		                  .block();
		assertThat(res).as("sequences equal").isTrue();

		res = Mono.sequenceEqual(Flux.just(1, 3), Flux.just(1, 2, 3))
		                  .block();
		assertThat(res).as("sequences not equal").isFalse();
	}
	...
	public void shouldCorrectlyDispatchComplexFlow() throws InterruptedException {
		...
		assertThat(latch.getCount()).as("Must have counted 4 elements").isZero();
	...
	public void testParallelWithJava8StreamsInput() throws InterruptedException {
		...
		assertThat(countDownLatch.getCount()).as("latch count").isZero();
	}
	...
	public void testBeyondLongMaxMicroBatching() throws InterruptedException {
		...
		assertThat(countDownLatch.getCount()).as("Count max: "+ tasks.size()).isZero();
	}
	...
	public void shouldWindowCorrectly() throws InterruptedException {
		...
		assertThat(endLatch.getCount()).as("endLatch count").isZero();
	}
	...
	public void consistentMultithreadingWithPartition() throws InterruptedException {
		...
		assertThat(latch.getCount()).as("latch count").withFailMessage("Not totally dispatched").isZero();
		supplier1.dispose();
		supplier2.dispose();
	}
	...
	public void subscribeOnDispatchOn() throws InterruptedException {
		...
		assertThat(latch.await(30, TimeUnit.SECONDS))
				.as("latch await")
				.withFailMessage("Not totally dispatched")
				.isTrue();
	}
	@Test
...
// code by @masterav10
	...
	public void endLessTimer() throws InterruptedException, TimeoutException {
		...
		assertThat(times).hasSize(tasks);
		...
		for (int i = 1; i < times.size(); i++) {
			...
			assertThat((double) (time - prev)).isLessThanOrEqualTo(delayMS * 1.2);
		}
	}
	...
	public void testThrowWithoutOnErrorShowsUpInSchedulerHandler() {
		...
		assertThat(handled.get()).withFailMessage("Uncaught error not handled").isTrue();
		if (failure.get() != null) {
			fail(failure.get());
		}
	}
