...
public class MonoSequenceEqualTest {
	...
	public void differenceCancelsBothSources() {
		...
		assertThat(sub1.get()).withFailMessage("left not cancelled").isTrue();
		assertThat(sub2.get()).withFailMessage("right not cancelled").isTrue();
	}
	...
	public void cancelCancelsBothSources() {
		...
		assertThat(sub1.get()).withFailMessage("left not subscribed").isNotNull();
		assertThat(cancel1.get()).withFailMessage("left not cancelled").isTrue();
		assertThat(sub2.get()).withFailMessage("right not subscribed").isNotNull();
		assertThat(cancel2.get()).withFailMessage("right not cancelled").isTrue();
	}
	...
	public void doubleCancelCancelsOnce() {
		...
		assertThat(sub1.get()).withFailMessage("left not subscribed").isNotNull();
		assertThat(cancel1.get()).as("left cancelled").isEqualTo(1);
		assertThat(sub2.get()).withFailMessage("right not subscribed").isNotNull();
		assertThat(cancel2.get()).as("right cancelled").isEqualTo(1);
	}
	...
	public void subscribeInnerOnce() {
		...
		assertThat(innerSub1.intValue()).withFailMessage("left has been subscribed multiple times")
		                                .isEqualTo(1);
		assertThat(innerSub2.intValue()).withFailMessage("right has been subscribed multiple times")
		                                .isEqualTo(1);
	}
